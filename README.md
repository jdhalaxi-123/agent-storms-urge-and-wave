...
2) 引导填 DeepSeek API Key
3) **手动填课题组 FTP**：自动打开记事本编辑 .env 的 FTP 四行，保存后自动测连接
4) 自动跑安装体检（依赖/地图数据/出图/配置）
5) 问你要不要直接启动（回车即开，浏览器打开 http://localhost:7860）
```

> 地图岸线数据（23 MB）已随仓库提供，**无需联网另外下载**。
> FTP 采用**手动填 .env** 的方式（自动向导容易被交互/编码问题坑到），
> 具体四行见下方「配置课题组 FTP」。
...
| --- | --- |
| `2-启动.bat` | 打开对话页面 |
| `5-配置FTP.bat` | **手动**配置 FTP（自动开记事本编辑 .env + 连通性自检） |
| `4-设置自动下载.bat` | 让数据每天自动更新（每 30 分钟检查一次） |
| `3-环境自检.bat` | 出问题时生成《环境自检报告.md》 |
| `scripts/check_install.py` | 安装体检（依赖/文件/地图/出图/配置） |
| `scripts/check_ftp.py` | FTP 专检（登录 + 各数据最新到哪天） |
| `6-卸载-Windows.bat` | 卸载（先列清单，输入 YES 才执行） |

#### 配置课题组 FTP（手动，推荐）

编辑项目根目录的 `.env`，把下面四行写好（`5-配置FTP.bat` 会自动打开这个文件）：

```
FTP_HOST=120.42.36.229
FTP_PORT=22210          # 注意不是默认的 21
FTP_USER=你的账号
FTP_PASS=你的密码
```

注意：等号后**不加引号、行尾不留空格**；密码以 `!` 结尾时直接复制粘贴（在 cmd 里手打会被吞）。
保存后运行 `scripts/check_ftp.py` 验证，看到 `③ 登录 ✅` 即成功。
